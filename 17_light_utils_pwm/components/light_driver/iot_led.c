...
{
    esp_err_t ret = ESP_OK;
    MDF_ERROR_CHECK(g_light_config == NULL, ESP_FAIL, "iot_led_init() must be called first");
#ifdef CONFIG_SPIRAM_SUPPORT
    MDF_ERROR_CHECK(gpio_num != GPIO_NUM_16 || gpio_num != GPIO_NUM_17, ESP_FAIL,
...
{
    MDF_ERROR_CHECK(g_light_config == NULL, ESP_FAIL, "iot_led_init() must be called first");
    MDF_ERROR_CHECK(dst == NULL, ESP_FAIL, "dst should not be NULL");
...
{
    MDF_ERROR_CHECK(g_gamma_table == NULL, ESP_FAIL, "iot_led_init() must be called first");
